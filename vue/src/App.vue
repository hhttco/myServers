  ...
  socket.onmessage = function (event) {
    try {
      ...
      data.value = res.map((host) => {
        ...
        return {
          ...host,
          status: nowtime - host.TimeStamp <= 60 ? 1 : 0,
        }
      })
  ...
  <div class="max-container">
    <div class="header">
      <a class="logo" href="#">
        ...
        <small style="font-weight: 400; opacity: 0.8"> ｜ {{ $t('title') }}</small>
      </a>
      <a-space>
        <HeaderLocale />
            ...
            <a-col :span="10" :xs="24" :sm="24" :md="10" :lg="10" :sl="10">
              <div class="detail-item-list">
                ...
                <div class="detail-item">
                  ...
                  <div class="value">
                    {{ formatBytes(item.State.SwapUsed) }} / {{ formatBytes(item.Host.SwapTotal) }}
                  </div>
                </div>
                <div class="detail-item">
    ...
    <a-modal v-model:visible="editVisible" :footer="false" :hide-title="true" width="360px">
      ...
      <div class="akile-modal-action">
        <a-button type="primary" :long="true" @click="handleEditHost">{{
          $t('edit-host-btn')
        }}</a-button>
      </div>
    </a-modal>
